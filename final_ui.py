...
.controls{background:#FFF;border:1px solid #E5E5E5;border-radius:10px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;box-shadow:0 1px 3px rgba(0,0,0,.04)}
.ctrl-grp{display:flex;flex-direction:column;gap:5px}
.ctrl-grp.grow{flex:1;min-width:180px}
...
.search-wrap .iconify{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:#999;font-size:15px;pointer-events:none}
.search-wrap .ctrl-in{padding-left:36px;width:100%}
...
.card-img .ph{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#bbb;font-size:28px;pointer-events:none}
...
<div class="search-wrap"><label class="ctrl-label">Search</label><span class="iconify" data-icon="lucide:search"></span><input class="ctrl-in" id="sr" placeholder="Narrow down... e.g. IPL final, GPT-5, SpaceX Starship"></div>
...
function init() {
    ...
    sb.addEventListener("click", function(e) {
        ...
        if (item) sel(item, item.dataset.domain);
    });
    dm.onchange = function() {
        var opt = dm.options[dm.selectedIndex];
        var fakeEl = {classList:{add:function(){}}};
        sel(fakeEl, opt.value);
        var items = sb.querySelectorAll(".sb-item");
        for (var i = 0; i < items.length; i++) {
            items[i].classList.remove("active");
            if (items[i].dataset.domain === opt.value) items[i].classList.add("active");
        }
    };
    document.getElementById("st").onchange = function() {
        var sub = this.options[this.selectedIndex].text;
        log("info", "Sub-topic: " + sub);
    };
...
function showWelcome() {
    document.getElementById("nc").innerHTML =
        ...
        '<p>Select a domain from the sidebar, pick a sub-topic, then search for exactly what you need.</p>' +
        ...
        '</div>' +
        '<button class="retry-btn" id="retryBtn">Load Trending News</button>' +
        '</div>';
    ...
    document.getElementById("nc").addEventListener("click", function handler(e) {
        ...
        if (btn) {
            ...
            document.getElementById("nc").removeEventListener("click", handler);
        }
    });
}
function quickGo(domain, sub, search) {
    ...
    for (var i = 0; i < items.length; i++) {
        items[i].classList.remove("active");
        if (items[i].dataset.domain === domain) items[i].classList.add("active");
    }
...
function populateSubtopics(domain) {
    ...
    if (subs) {
        ...
        for (var i = 0; i < keys.length; i++) {
            st.innerHTML += '<option value="' + esc(subs[keys[i]]) + '">' + esc(keys[i]) + '</option>';
        }
    }
}
function sel(el, d) {
                ...
                fetch("/api/news?domain=" + encodeURIComponent(d) + "&subtopic=&days=1")
                    ...
                    .then(function(data) {
                        if (Array.isArray(data) && data.length > 0) {
                            ...
                            for (var j = 0; j < items.length; j++) items[j]._domain = d;
                            all = all.concat(items);
                        }
                        done++;
                        log("ok", d + ": " + (Array.isArray(data) ? data.length : 0) + " found");
...
function finishTrending(all) {
    ...
    var html = '<div class="trending-hdr"><h2>&#128293; Trending Now</h2><button class="back-btn" style="margin:0" id="refreshBtn">Refresh</button></div>';
    html += '<p class="trending-sub">Top stories from across all domains. Pick a domain to dive deeper.</p><div class="grid">';
    for (var i = 0; i < all.length; i++) {
        var a = all[i];
        var desc = (a.content || "").substring(0, 120);
        if (desc.length >= 120) desc += "...";
        html += '<div class="card" data-url="' + esc(a.url || "") + '"><div class="card-img">';
        if (a.image_url) html += '<img src="' + esc(a.image_url) + '">';
        html += '<div class="ph"><span class="iconify" data-icon="lucide:image-off"></span></div></div>';
        html += '<div class="card-body"><div class="tr-tag">' + esc(a._domain || "") + '</div>';
        html += '<div class="card-title">' + esc(a.title || "") + '</div>';
        html += '<div class="card-desc">' + esc(desc) + '</div>';
        html += '<div class="card-meta"><span>' + esc(a.author || "News") + '</span><span class="read">Read &#8594;</span></div></div></div>';
    }
    html += '</div>';
...
function setD(n) {
    ...
    for (var i = 0; i < chips.length; i++) chips[i].classList.toggle("active", parseInt(chips[i].dataset.d) === n);
    if (curD) go();
}
...
function go() {
    ...
    var search = document.getElementById("sr").value;
    var query = subVal;
    if (search && subVal) query = subVal + " " + search;
    else if (search) query = search;
    log("warn", "Fetching " + curD + (query ? " (" + query + ")" : "") + "...");
    ...
    fetch("/api/news?domain=" + encodeURIComponent(curD) + "&subtopic=" + encodeURIComponent(query) + "&days=" + days)
        .then(function(r) { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
        .then(function(data) { render(data); log("ok", "Found " + data.length + " articles"); })
        .catch(function(e) { log("err", "Failed: " + e.message); showWelcome(); });
}
function render(data) {
    if (!data || data.length === 0) {
        document.getElementById("nc").innerHTML = '<div class="back-btn" id="backBtn">Back to Trending</div><div class="welcome"><h2>No articles found</h2><p>Try a different sub-topic or search term.</p></div>';
    ...
    var html = '<div class="back-btn" id="backBtn">Back to Trending</div>';
    ...
    for (var i = 0; i < dates.length; i++) {
        ...
        for (var j = 0; j < groups[date].length; j++) {
            var a = groups[date][j];
            var desc = (a.content || "").substring(0, 150); if (desc.length >= 150) desc += "...";
            html += '<div class="card" data-url="' + esc(a.url || "") + '"><div class="card-img">';
            if (a.image_url) html += '<img src="' + esc(a.image_url) + '">';
            html += '<div class="ph"><span class="iconify" data-icon="lucide:image-off"></span></div></div>';
            html += '<div class="card-body"><div class="card-dom">' + esc(curD) + '</div>';
            html += '<div class="card-title">' + esc(a.title || "") + '</div>';
            html += '<div class="card-desc">' + esc(desc) + '</div>';
            html += '<div class="card-meta"><span>' + esc(a.author || "News") + '</span><span class="read">Read &#8594;</span></div></div></div>';
        }
...
function dl() {
    ...
    fetch("/api/generate-pdf?domain=" + encodeURIComponent(curD) + "&subtopic=" + encodeURIComponent(query) + "&days=" + days)
        ...
        .catch(function(e) { log("err","PDF error"); });
}
init();
</script>
...
with open("templates/index.html", "r", encoding="utf-8") as f:
    check = f.read()
dom_count = len(json.loads(dm_json))
if "var DM =" in check and size > 10000:
    print("SUCCESS! File: " + str(size) + " bytes")
    print("Domains loaded: " + str(dom_count))
    sub_count = sum(len(v) for v in json.loads(dm_json).values())
    print("Total sub-topics: " + str(sub_count))
else:
    print("ERROR! Size: " + str(size))
