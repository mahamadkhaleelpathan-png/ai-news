...
.no-img{width:100%;height:170px;background:linear-gradient(135deg,#E8E4DD,#D5D0C8);display:flex;align-items:center;justify-content:center;color:#aaa;font-size:12px}
...
.welcome-icon{font-size:56px;margin-bottom:16px;opacity:.3}
.welcome h2{font-family:Playfair Display,serif;font-size:24px;color:#1A1A1A;margin-bottom:8px}
.welcome p{color:#888;font-size:14px;max-width:400px;margin-bottom:24px}
.quick-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;width:100%;max-width:500px}
.quick-btn{padding:12px 8px;border-radius:8px;border:1px solid #E5E5E5;background:#fff;font-size:12px;font-weight:600;color:#555;cursor:pointer;transition:all .2s;text-align:center}
.quick-btn:hover{border-color:#C8A96A;color:#C8A96A;background:rgba(200,169,106,.05)}
.quick-btn span{display:block;font-size:10px;color:#aaa;font-weight:400;margin-top:2px}
.retry-btn{margin-top:20px;padding:10px 24px;border-radius:8px;border:none;background:#C8A96A;color:#fff;font-size:13px;font-weight:600;cursor:pointer;transition:all .2s}
...
function init() {
    ...
    log("info", "System ready - " + Object.keys(DM).length + " domains loaded");
    showWelcome();
    loadTrending();
}
function showWelcome() {
    document.getElementById("nc").innerHTML = '<div class="welcome">' +
        '<div class="welcome-icon"><span class="iconify" data-icon="lucide:compass"></span></div>' +
        '<h2>Welcome to TrendScope</h2>' +
        '<p>Select a domain from the sidebar or pick a quick topic below to start exploring trending news.</p>' +
        '<div class="quick-grid">' +
        '<div class="quick-btn" onclick="quickGo(\'Artificial Intelligence (AI)\',\'LLMs & GPT\')">AI & LLMs<span>Latest models</span></div>' +
        '<div class="quick-btn" onclick="quickGo(\'Technology\',\'Software & Apps\')">Tech<span>Apps & Software</span></div>' +
        '<div class="quick-btn" onclick="quickGo(\'Sports\',\'Cricket\')">Cricket<span>IPL & matches</span></div>' +
        '<div class="quick-btn" onclick="quickGo(\'Crypto\',\'Bitcoin & Ethereum\')">Crypto<span>Bitcoin & ETH</span></div>' +
        '<div class="quick-btn" onclick="quickGo(\'Science\',\'Space (NASA/SpaceX)\')">Space<span>NASA & SpaceX</span></div>' +
        '<div class="quick-btn" onclick="quickGo(\'Business\',\'Startups\')">Startups<span>Funding & news</span></div>' +
        '</div>' +
        '<button class="retry-btn" onclick="loadTrending()"><span class="iconify" data-icon="lucide:refresh-cw"></span> Load Trending News</button>' +
        '</div>';
}
function quickGo(domain, sub) {
    document.getElementById("dm").value = domain;
    document.getElementById("st").value = sub;
    curD = domain;
    document.querySelectorAll(".sb-item").forEach(function(e){
        e.classList.toggle("active", e.textContent === domain);
    });
    go();
}
function loadTrending() {
    var nc = document.getElementById("nc");
    nc.innerHTML = '<div class="loading"><div class="spinner"></div>Fetching trending news from all domains...</div>';
    log("info", "Loading trending news...");
            ...
            fetch("/api/news?domain=" + encodeURIComponent(d) + "&subtopic=&days=1")
                ...
                .then(function(data) {
                    if (Array.isArray(data)) {
                        ...
                        items.forEach(function(a) { a._domain = d; });
                        all = all.concat(items);
                    }
                    done++;
                    log("ok", d + ": " + (Array.isArray(data) ? data.length : 0) + " articles");
...
function finishTrending(all) {
    if (all.length === 0) {
        log("warn", "No trending news loaded");
        showWelcome();
        return;
    }
    log("ok", "Total: " + all.length + " trending articles");
    var html = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"><h2 style="font-family:Playfair Display,serif;font-size:22px;font-weight:700;color:#1A1A1A">&#128293; Trending Now</h2><button class="back-btn" style="margin:0" onclick="loadTrending()"><span class="iconify" data-icon="lucide:refresh-cw"></span> Refresh</button></div>';
    html += '<p style="font-size:13px;color:#888;margin-bottom:20px">Top stories from across all domains. Click any card to read, or pick a domain to dive deeper.</p>';
    html += '<div class="grid">';
    all.forEach(function(a) {
        ...
        html += '<div class="card-body"><div class="tr-tag">' + (a._domain || "") + '</div>';
        html += '<div class="card-title">' + (a.title || "") + '</div>';
        html += '<div class="card-desc">' + desc + '</div>';
        html += '<div class="card-meta"><span>' + src + '</span><a href="' + (a.url || "#") + '" target="_blank" onclick="event.stopPropagation()">Read &#8594;</a></div></div></div>';
    });
    html += '</div>';
...
function go(){if(!curD){log("err","Select a domain first");return;}var sub=document.getElementById("st").value;log("warn","Fetching "+curD+(sub?" ("+sub+")":"")+"...");document.getElementById("nc").innerHTML='<div class="loading"><div class="spinner"></div>Loading...</div>';fetch("/api/news?domain="+encodeURIComponent(curD)+"&subtopic="+encodeURIComponent(sub)+"&days="+days).then(function(r){return r.json();}).then(function(data){render(data);log("ok","Found "+data.length+" articles");}).catch(function(e){log("err","Fetch failed");showWelcome();});}
function render(data){
    if(!data.length){
        document.getElementById("nc").innerHTML='<div class="back-btn" onclick="loadTrending()"><span class="iconify" data-icon="lucide:arrow-left"></span> Back to Trending</div><div class="welcome"><div class="welcome-icon"><span class="iconify" data-icon="lucide:search-x"></span></div><h2>No articles found</h2><p>Try a different sub-topic or increase the date range.</p><button class="retry-btn" onclick="loadTrending()"><span class="iconify" data-icon="lucide:arrow-left"></span> Back to Trending</button></div>';
        return;
    }
    var groups={};data.forEach(function(a){var d=a.date_group;if(!groups[d])groups[d]=[];groups[d].push(a);});
...
with open("templates/index.html", "w", encoding="utf-8") as file:
    file.write(html)
print("FIXED - No more empty whitespace!")
print("- Welcome screen shows immediately (no blank space)")
print("- 6 quick-access buttons to jump to popular topics")
print("- Trending loads in background with staggered requests")
print("- If trending fails, welcome screen stays (never blank)")
